...
Deno.serve(async (request: Request) => {
  ...
  if (!sessionSecret) return jsonError(503, "CONFIG_MISSING", "Authentication is not configured.", cors.headers);
  const authorization = request.headers.get("Authorization") ?? "";
  const match = /^Bearer\s+(.+)$/i.exec(authorization);
  if (!match) return jsonError(401, "AUTHORIZATION_INVALID", "Session token is required.", cors.headers);
  try {
    const claims = await verifySessionToken({ token: match[1], secret: sessionSecret, nowSeconds: Math.floor(Date.now() / 1000) });
    return jsonResponse(200, { ok: true, session: { expiresAt: claims.exp, source: "telegram", user: claims.user } }, cors.headers);
  } catch (error) {
    if (error instanceof SessionTokenError) return jsonError(401, error.code, "Session token is invalid or expired.", cors.headers);
    return jsonError(500, "INTERNAL_ERROR", "Internal session error.", cors.headers);
  }
});
