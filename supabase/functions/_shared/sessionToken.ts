...
export async function signSessionToken(input: {
  ...
  const signature = jsonToBase64UrlBytes(
    await hmacSha256(input.secret, signingInput),
  );
...
export async function verifySessionToken(input: {
  ...
  const expectedSignature = jsonToBase64UrlBytes(
    await hmacSha256(input.secret, signingInput),
  );
  if (
    !constantTimeEqual(
      new TextEncoder().encode(expectedSignature),
      new TextEncoder().encode(encodedSignature),
    )
  ) {
    throw new SessionTokenError(
  ...
  const header = safeDecode<Record<string, unknown>>(
    encodedHeader,
  ...
  if (
    ...
    throw new SessionTokenError(
      "TOKEN_HEADER_INVALID",
      "Session token header is invalid.",
    );
  }

  const claims = safeDecode<Record<string, unknown>>(
    encodedPayload,
...
function validateClaims(
  ...
  if (claims.iss !== ISSUER) {
    throw new SessionTokenError("ISSUER_INVALID", "Session issuer is invalid.");
  }
  if (claims.aud !== AUDIENCE) {
    throw new SessionTokenError(
      "AUDIENCE_INVALID",
      "Session audience is invalid.",
    );
  }
  if (typeof claims.sub !== "string" || claims.sub.length === 0) {
    throw new SessionTokenError(
      "SUBJECT_INVALID",
      "Session subject is invalid.",
    );
  }
  if (!Number.isSafeInteger(claims.iat) || Number(claims.iat) <= 0) {
    throw new SessionTokenError("IAT_INVALID", "Session issued-at is invalid.");
  }
  if (
    !Number.isSafeInteger(claims.exp) ||
    Number(claims.exp) <= Number(claims.iat)
  ) throw new SessionTokenError("EXP_INVALID", "Session expiry is invalid.");
  if (Number(claims.exp) <= nowSeconds) {
    throw new SessionTokenError("TOKEN_EXPIRED", "Session token is expired.");
  }
  if (Number(claims.exp) - Number(claims.iat) > MAX_TTL_SECONDS) {
    throw new SessionTokenError("TTL_INVALID", "Session TTL is invalid.");
  }
  if (typeof claims.jti !== "string" || claims.jti.length === 0) {
    throw new SessionTokenError("JTI_INVALID", "Session id is invalid.");
  }
  if (claims.source !== SOURCE) {
    throw new SessionTokenError("SOURCE_INVALID", "Session source is invalid.");
  }
  validateUser(claims.user);
  if (claims.sub !== String((claims.user as NormalizedTelegramUser).id)) {
    throw new SessionTokenError(
      "SUBJECT_INVALID",
      "Session subject is invalid.",
    );
...
function validateUser(user: unknown): asserts user is NormalizedTelegramUser {
  if (!user || typeof user !== "object" || Array.isArray(user)) {
    throw new SessionTokenError("USER_INVALID", "Session user is invalid.");
  }
  ...
  if (
    typeof candidate.firstName !== "string" || candidate.firstName.length === 0
  ) throw new SessionTokenError("USER_INVALID", "Session user is invalid.");
}
