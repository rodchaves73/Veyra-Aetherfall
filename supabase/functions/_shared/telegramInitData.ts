...
export async function validateTelegramInitData(input: {
  ...
  if (!hash) {
    throw new TelegramInitDataError(
      "HASH_REQUIRED",
      "Telegram initData hash is required.",
    );
  }
  ...
  if (!authDateRaw) {
    throw new TelegramInitDataError(
      "AUTH_DATE_REQUIRED",
      "Telegram auth_date is required.",
    );
  }
...
function normalizeTelegramUser(value: unknown): NormalizedTelegramUser {
  ...
  if (typeof user.is_premium === "boolean") {
    normalized.isPremium = user.is_premium;
  }
  if (user.is_premium !== undefined && typeof user.is_premium !== "boolean") {
    throw new TelegramInitDataError(
      "USER_PREMIUM_INVALID",
      "Telegram premium flag is invalid.",
    );
  }
