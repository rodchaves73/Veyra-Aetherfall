...
## ADR-011 | Sessão da aplicação

**Decisão:** usar Supabase Edge Functions para emitir e verificar uma sessão curta própria do Veyra assinada com HMAC-SHA-256.

**Regras:**

- A sessão Veyra não é um Supabase Auth JWT.
- A sessão Veyra não deve ser usada diretamente para acessar tabelas via RLS.
- A sessão é destinada a chamadas futuras para Edge Functions com autoridade server-side do Veyra.
- O token não inclui bot token, segredo de sessão, `initData` ou `hash`.
- O token futuro deve ficar somente em memória no cliente, não em storage persistente.
- A sessão tem TTL curto e não resolve revogação ou replay prevention persistente sem armazenamento server-side.
- Qualquer migração futura para Supabase Auth ou outro provedor de sessão precisa de ADR específica.
