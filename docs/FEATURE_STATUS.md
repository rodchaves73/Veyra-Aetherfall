...
| Telegram initData validator | PREPARED | Supabase Edge Function/shared code | Não | Preparada em código | Fase 2B | Validador server-side criado, ainda sem deploy/teste real. |
| Veyra app session signer | PREPARED | Supabase Edge Function/shared code | Não | Preparada em código | Fase 2B | Emite sessão curta própria do Veyra; não é Supabase Auth JWT. |
| Session verifier | PREPARED | Supabase Edge Function/shared code | Não | Preparada em código | Fase 2B | Verifica Bearer token Veyra; ainda sem deploy. |
| Telegram authentication production | BLOCKED | Server-side planejado | Não | Parcial/preparada | Fase 2B | Bloqueada até secrets, deploy, teste real e integração frontend. |
| Frontend auth integration | PLANNED | Frontend | Não | Não | Fase 2B | Não implementada nesta tarefa; token futuro deve ficar somente em memória. |
| Supabase Auth integration | DEFERRED | Não implementado | Não | Não | ADR futura | Sessão atual é própria do Veyra, não Supabase Auth. |
