...
import type { PlayerBootstrapError, PlayerBootstrapResult, VeyraPlayer, VeyraPlayerAccountStatus } from './playerTypes';
...
const mapPlayer = (value: unknown): VeyraPlayer | null => {
  ...
  if (!id || !telegramUserId || !displayName || !isAccountStatus(accountStatus) || !createdAt || !updatedAt || !lastSeenAt) return null;
  return { id, telegramUserId, displayName, accountStatus, createdAt, updatedAt, lastSeenAt };
};
